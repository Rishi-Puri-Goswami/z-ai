...
class SearchApi {
    ...
    async *images(keywords, region = "wt-wt", safesearch = "moderate", timelimit = null, size = null, color = null, type_image = null, layout = null, license_image = null) {
        if (!keywords) {
        ...
        const cache = new Set();
        for (let _ = 0; _ < 10; _++) {
            const resp = await this._getUrl("GET", "https://duckduckgo.com/i.js", payload);
            if (!resp) {
                break;
            }
            try {
    ...
    async *text(keywords, region = "wt-wt", safesearch = "moderate", timeLimit = null) {
        if (!keywords) {
            throw new Error("Keywords are mandatory");
        }
        const vqd = await this._getVqd(keywords);
